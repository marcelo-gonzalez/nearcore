...
fn chunk_state_roots(block: BlockView) -> Vec<CryptoHash> {
    let mut chunks = block.chunks;
...
async fn get_prev_epoch_state_roots(
    ...
    Ok(chunk_state_roots(prev_epoch_last_block_response))
}
...
async fn get_current_epoch_state_roots(
    ...
    shard_layout: &ShardLayout,
) -> anyhow::Result<Option<Vec<CryptoHash>>> {
        ...
        match rpc_client.block_by_id(BlockId::Height(height)).await {
            Ok(block) => {
                ...
                if num_new_chunks.iter().all(|(_shard_id, new_chunks)| *new_chunks >= 2) {
                    return Ok(Some(chunk_state_roots(block)));
                }
            }
            Err(e) => {
...
// Should just check if the epoch ID is new before doing that.
