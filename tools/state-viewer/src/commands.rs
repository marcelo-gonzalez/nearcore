    ...
    for height in start_height..end_height {
        ...
        for c in block.chunks().iter() {
            ...
            for (i, tx) in chunk.transactions().iter().enumerate() {
                if (show_signer && &tx.transaction.signer_id == &account_id)
                    || (show_receiver && &tx.transaction.receiver_id == &account_id)
                {
                    println!(
            ...
            for (i, r) in chunk.receipts().iter().enumerate() {
                if (show_signer && &r.predecessor_id == &account_id)
                    || (show_receiver && &r.receiver_id == &account_id)
                {
                    println!(
                    ...
                    let outcome =
                        get_execution_outcome(&chain, &r.receipt_id).with_context(|| {
                            format!("Could not find execution outcome for receipt {}", &r.receipt_id)
                        })?;
                    println!("{:?}\n------------------------\n", &outcome.outcome_with_id);
                }
            }
