...
def check_num_txs(source_node, target_node):
    ...
    assert total_source_txs <= total_target_txs, (total_source_txs,
                                                  total_target_txs)
    logger.info(f'passed. num source txs:  {total_source_txs} num target txs:  {total_target_txs}')
...
def send_traffic(near_root, source_nodes, traffic_data, callback):
    ...
    k = key.Key.from_random('test1')
    test1_new_key = AddedKey(k)
    send_add_access_key(source_nodes[1], source_nodes[1].signer_key, k,
    ...
    test1_deleted_height = None
    test1_readded_key = None
    implicit_added = None
    ...
    for height, block_hash in utils.poll_blocks(source_nodes[1],
        ...
        if test1_deleted_height is None:
            traffic_data.send_transfers(source_nodes, block_hash_bytes)
        else:
        ...
        if test1_deleted_height is None and test1_new_key.inited(
        ) and height - start_source_height >= 15:
            send_delete_access_key(source_nodes[1], test1_new_key.key,
                                   source_nodes[1].signer_key,
                                   test1_new_key.nonce + 1, block_hash_bytes)
            test1_new_key.nonce += 1
            test1_deleted_height = height

        if test1_readded_key is None and test1_deleted_height is not None and height - test1_deleted_height >= 5:
            send_add_access_key(source_nodes[1], test1_new_key.key,
                                source_nodes[1].signer_key,
                                test1_new_key.nonce + 1, block_hash_bytes)
            test1_readded_key = AddedKey(source_nodes[1].signer_key)
            test1_new_key.nonce += 1

        if test1_readded_key is not None:
            test1_readded_key.send_if_inited(
